...
class Data_augmentation:
    ...
    def image_augment(self, save_path): 
        ...
        for angle in range(0, 360, 10):
            if angle != 0:
                img_rot = self.rotate(img, angle)
                name_int = self.name[:len(self.name)-4]        
                cv2.imwrite(save_path +'%s' %str(name_int)+'_rot{}.jpg'.format(angle), img_rot)
...
labelpath = "./traindata/"
# print(os.listdir("./traindata/"))
ClassesLabel = os.listdir(labelpath)
totalfiles = 0
# for angle in range(-90,91,1):
#     if angle != 0:
#         print(angle)
for ctl in ClassesLabel:
    Data_augmentation.main("./traindata/{}".format(ctl), "./traindata/{}/".format(ctl))
